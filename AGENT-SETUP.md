...
**On the plugin:** `claude plugin marketplace update llm-peer-review && claude plugin update tk@llm-peer-review`, restart or `/reload-plugins`, then invoke the `tk:upgrade` skill in every project on the plugin (the session notice asks for it in each project that is behind; a project with nothing behind is told so in one line). It audits the project's own `.claude/` files and `CLAUDE.md`, plus `.claude/settings.local.json`, `.gitignore`, `.gitattributes`, `artifacts/README.md`, and a migration record git still tracks, against the conventions that changed since the project's last audited version (`docs/CONVENTIONS.md`); the rules-file check (C-7) runs on every upgrade. Findings go through the normal M2 audit and auto-fix loop, and it stops once to ask before editing any prompt file. The update itself never touches the project.
...
- **`tk:setup` exits 3** - It needs a decision (dirty tree, a migration outside a git repository, locally modified toolkit files, unknown provenance) and touched nothing; show the user its list and rerun with `--force` only when they say yes
