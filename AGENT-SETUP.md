# AI Agent Setup Instructions (v7.3.0)
...
**What's new in v7.3.0:** A regression audit on top of v7.2.0 (#184). Everything that changed from v6.3.3 to v7.2.0 was read again with receipts, and the 22 regressions it found are fixed; most came from the 7.0.0 plugin build rewriting names and permissions for the plugin path and leaving the copy-install path behind. A `tk:review` typed with no range now starts from the newest plan's `**Start commit:**` while any commit after it is unpushed, never capped, and says which source it used and why; a plan whose commits are all pushed stops the run and names the range to pass. Copy-installs get the four permission rows the 7.x prompts call (`mktemp -d`, `session-init.js --scope`, `setup-project.js`, `upgrade-audit.js`) and drop the old `.claude/rules/html-outputs.md` on their next setup run; the rules seed and the shared scripts say the same thing on both installs; `tk:review-full` reviews Operations with its own worker again and runs without a plan file; the manual explains its plugin-root paths. Maintainers get `scripts/setup/headless-session.sh`, a headless session under a scratch home. To update: `claude plugin marketplace update llm-peer-review`, then `claude plugin update tk@llm-peer-review`, restart Claude Code, and run `tk:upgrade` in each project (C-7 carries the new rules text).

**What was new in v7.2.0:** Fixes on top of v7.1.0 (#178 to #183). A `tk:review` chained from `tk:execute` sees the committed work again: `tk:execute` writes the plan's `**Start commit:**` and hands over `<start>..HEAD`, and a review typed on a clean tree covers the newest 20 unpushed commits and names its range in its first line (changed in v7.3.0: a review typed with no range starts from the newest plan's start commit while any commit after it is unpushed, and says why). `tk:setup` backs up both settings files on every run that changes them, names the rows it adds and removes, and never adds back a row this working copy was already offered. `tk:upgrade` runs C-9, C-10 and C-11 on every upgrade with receipts that check what the detector checks. The pre-push tripwire passes whole-value placeholders and scans what the push destination lacks. Temp files, debate sessions, `tk:index` and GitLab bodies no longer stop for approval in default permission mode. Rollback, teammate and automatic-update steps are documented in `toolkit-reference.md` (run a rollback from a folder that is not a project), and `upgrade-audit.js --rollback-to <version>` lowers a project's recorded version first. To update: `claude plugin marketplace update llm-peer-review`, then `claude plugin update tk@llm-peer-review`, restart Claude Code, and run `tk:upgrade` in each project.

**What was new in v7.1.0:** Safe distribution, on top of v7.0.0 (#172 to #177). The plugin now installs from the release tag, not from `main`, and a maintainer release gate keeps a tag from going out while a suite is red or `plugin/` changed without a version bump. Each session start says when the project's recorded version and the running plugin differ, or when an old copy-install still sits beside the plugin, and the pre-push tripwire blocks a push from a plugin older than the project's record. A project's own `.env.local` is read again on the plugin (environment first, then the project file, then `~/.claude/plugins/.env.local`), and only the toolkit's key and model variables are taken from it. `tk:setup` seeds files written for a downstream plugin project, recognizes copy-installs old enough to have no manifest, and prints an undo line that restores the tree when followed in order. `tk:upgrade` gains repair checks for projects migrated on 7.0.x: C-9 (permission rows, both directions, including rows the 7.0.0 migration lost; `defaultMode` is asked, never changed), C-10 (stale seeded lines, lost `.gitignore` and `.gitattributes` lines, a migration record git still tracks) and C-11 (toolkit names without the `tk:` prefix); C-7 now runs on every upgrade and fires only when the rules text differs from the seed. To update: `claude plugin marketplace update llm-peer-review`, then `claude plugin update tk@llm-peer-review`, restart Claude Code, and run `tk:upgrade` in each project.

**Older releases:** v7.0.1 (the tripwire and a mail client's mangled links), v7.0.0 (the toolkit as a Claude Code plugin, typed finder agents, and the upgrade audit), v6.3.3 (six credential formats for the pre-push tripwire), v6.3.2 (the standing cycle page), v6.3.1 (the receipt files and the standing review page after the fix loop), v6.3.0 (what a review finding says, and the standing review page), v6.2.0 (the design workflow), v6.1.1 (a plan-shell fix), v6.1.0 (the correction ledger, the hosted page as the primary viewport, installer parity) and v6.0.0 (the auto loop itself) are described in [CHANGELOG.md](CHANGELOG.md). If the user is upgrading from v5.x or earlier, read the v6.0.0 section there first: it is the release that changed behavior.
