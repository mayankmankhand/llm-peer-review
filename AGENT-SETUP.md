# AI Agent Setup Instructions (v7.1.0)
...
- **A toolkit command asks for approval before one of its own scripts, in default permission mode** - A plugin command's permission for its scripts lasts only until the user's next message, so a script it runs after the user answers a question asks; saving a review receipt's output to a file asks too. Tell the user to approve it; a later release is to lift the first
