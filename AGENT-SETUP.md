...
- `.claude/agents/` (all worker definitions - the review finder, index mapper, and correction extractor, carrying their model, effort, and tool settings - always updated)
...
- `.claude/commands/` - all slash command definitions
- `.claude/agents/` - all worker definitions (review finder, index mapper, correction extractor)
...
- `.claude/scripts/pre-push-check.js` and `.claude/scripts/correction-ledger.js` - the pre-push tripwire and the correction ledger helper
...
1. Run `npm install --prefix .claude/scripts` if `.claude/scripts/node_modules` is missing (needed for `/ask-gpt`, `/ask-gemini`, `/review-browser`).
