# AI Agent Setup Instructions (v7.3.1)
...
**What's new in v7.3.1:** Follow-ups on top of v7.3.0 (#185 to #194, #198). `tk:setup` now writes permission rows for every plugin script, at this machine's plugin cache folder and at the stable `current` path, so a toolkit script no longer stops for approval after a mid-command reply or in a stage the loop starts on its own (measured: 0 toolkit-script prompts across a default-mode cycle). It also writes six ask rows, so every force-push form asks. `tk:upgrade` reports a missing ask row, removes retired permission rows in one finding while leaving the broad `Bash(npm install *)` alone, and skips dated or versioned history lines. The push check catches a secret closed by its own quote and warns when its hook form gets no ref lines. The debate scripts move to `openai` 7 and `@google/genai` 2, which need Node.js 22 or newer. `browse.js` uses the Windows temp folder. To update: `claude plugin marketplace update llm-peer-review`, then `claude plugin update tk@llm-peer-review`, restart Claude Code, run `tk:setup` again in each project (it adds the new rows and keeps yours), then `tk:upgrade`.

**What was new in v7.3.0:** A regression audit on top of v7.2.0 (#184). Everything that changed from v6.3.3 to v7.2.0 was read again with receipts, and the 22 regressions it found are fixed; most came from the 7.0.0 plugin build rewriting names and permissions for the plugin path and leaving the copy-install path behind. A `tk:review` typed with no range now starts from the newest plan's `**Start commit:**` while any commit after it is unpushed, never capped, and says which source it used and why; a plan whose commits are all pushed stops the run and names the range to pass. Copy-installs get the four permission rows the 7.x prompts call (`mktemp -d`, `session-init.js --scope`, `setup-project.js`, `upgrade-audit.js`) and drop the old `.claude/rules/html-outputs.md` on their next setup run; the rules seed and the shared scripts say the same thing on both installs; `tk:review-full` reviews Operations with its own worker again and runs without a plan file; the manual explains its plugin-root paths. Maintainers get `scripts/setup/headless-session.sh`, a headless session under a scratch home. To update: `claude plugin marketplace update llm-peer-review`, then `claude plugin update tk@llm-peer-review`, restart Claude Code, and run `tk:upgrade` in each project (C-7 carries the new rules text).
