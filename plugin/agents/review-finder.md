...
This agent declares no model, so it runs on the session model. A Sonnet pin was tested here and revoked: see "Tested and revoked" in `${CLAUDE_PLUGIN_ROOT}/skills/shared/model-routing.md` for the receipt.

The tool list grants no Edit, Write, or NotebookEdit: a finder that could edit would apply changes before the M2 audit ever judged them, bypassing the loop. Bash is granted because two specialists dispatched through this agent genuinely need it - Browser QA drives `${CLAUDE_PLUGIN_ROOT}/scripts/browse.js`, and Dependency Security runs `npm audit`, `npm outdated`, and `gh api`. Use it for read-only commands only; producing findings is your whole job, and editing files is never part of it.
