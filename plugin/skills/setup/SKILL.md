...
2. **Run the script** from the project root, once:
   ...
   It detects one of four situations and says which in its first lines: a fresh project; a copy-install with its manifest (every managed file is classified against the manifest hash); a copy-install without a manifest (an install from before v5.5.0, provenance unknown, recognized by `VERSION` beside `${CLAUDE_PLUGIN_ROOT}/commands/review.md` or, for an install from before `VERSION` was copied into projects, by the toolkit stamp in `.claude/rules/toolkit.md`); or a project already on the plugin (seed check only). On a migration it refuses to start on a dirty git tree, because the undo relies on git holding the tree as it was before the run.

3. **Read the exit code.**
   - **0, done.** Relay the report verbatim. When the run changed anything, it ends with a one-line undo that restores the project when followed left to right (tracked files to check out, then files to delete, then files to copy back from the backup folder or restore by hand) and, after a migration, with "Next: run /tk:upgrade".
   - **3, paged.** Relay what it found: locally modified toolkit files with their diff summaries, files of unknown provenance (including a `VERSION` file it names for removal, so the user can say if it is the project's own), or a dirty tree. State the recommended default (commit or stash first; on locally modified files, proceed, since each one is backed up and becomes an `/tk:upgrade` finding with the diff as its receipt). Ask one question. On a clear yes, rerun with `--force`; on anything else, stop.
   - **1, error.** Relay the message and stop. Do not retry blindly; the message names the cause.

4. **After a migration, hand off.** Say the things the report cannot: the first push will page on the settings change (M11 in `${CLAUDE_PLUGIN_ROOT}/skills/shared/hitl-loop.md`, which is what a new marketplace pointer in `.claude/settings.json` looks like to the tripwire); if the plugin was installed in this same session, `/reload-plugins` registers its commands and agents; the migration's locally modified files are now `/tk:upgrade` findings; the API keys need no move, because the debate and media scripts look for each key in a real environment variable first, then the project's own `.env.local` (from the working folder up to the git root, so the project's existing file keeps working), then `~/.claude/plugins/.env.local`; and from the next session on, a notice at session start asks for `/tk:upgrade` whenever the plugin is newer than the version this project was last audited at. Then announce the handoff in one line ("Setup complete, chaining into `/tk:upgrade` to audit your custom files against this version's conventions. Say \"no chaining\" to stop here.") and invoke `/tk:upgrade` through the Skill tool. Say "no chaining" stops here (M14).
