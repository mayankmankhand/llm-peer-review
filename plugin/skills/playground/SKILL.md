...
### When invoked directly (`/tk:playground` cold)
...
The calling command (e.g. `/tk:explore` vision mode) provides the option set, the comparison context, and a signal about whether interactive primitives would change the decision. Use that context to choose a pattern. This skill runs no shell command beyond the one `mktemp` that makes its folder, so anything else that needs a shell (the seed strings behind the design step's prototypes) is generated by the caller and passed in.

### Output
Always end the flow by writing the HTML as `playground.html` in that folder, then emit a clickable `file://<folder>/tk:playground.html` link in chat, with the folder path `mktemp` printed typed out in full. Do not try to open the file via `xdg-open` or similar - the clickable link is the agreed-upon mechanism.
...
**Rendered-prototypes variant** (dispatched by `/tk:explore`'s design step, see `${CLAUDE_PLUGIN_ROOT}/skills/shared/design-rules.md`): each of the three cards holds a working, self-contained prototype of one direction - real markup and inline CSS, scaled with a CSS transform so all three fit on one screen, with click-to-expand to full size as the fixed primitive. The dispatcher builds the three prototypes (explore.md's "Build the prototypes" step) and supplies them with their names; the seed strings behind them stay hidden. This variant is default-on (`${CLAUDE_PLUGIN_ROOT}/skills/shared/html-outputs.md`), so when the dispatch context says so, announce it without the skip offer. The payload carries the pick and the user's notes (sketch below). Still Pattern 1, so the static-default rule holds.
...
// Rendered prototypes (the /tk:explore design step)
function buildPayload() {
...
`/tmp/playground.*` folders accumulate over time. The skill never deletes them automatically (no risk of wiping an open browser tab).
