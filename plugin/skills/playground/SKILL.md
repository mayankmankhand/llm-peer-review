...
### Output
Always end the flow by writing the HTML as `playground.html` in that folder, then emit a clickable `file://<folder>/playground.html` link in chat, with the folder path `mktemp` printed typed out in full. Do not try to open the file via `xdg-open` or similar - the clickable link is the agreed-upon mechanism.
