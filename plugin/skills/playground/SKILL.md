...
The calling command (e.g. `/tk:explore` vision mode) provides the option set, the comparison context, and a signal about whether interactive primitives would change the decision. Use that context to choose a pattern. This skill has no Bash, so anything that needs a shell (the seed strings behind the design step's prototypes) is generated by the caller and passed in.
