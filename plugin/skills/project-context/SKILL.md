...
2. **Project rules** - Read these files if they exist:
   ...
   - Look for critical rules that subagents must follow (e.g., the auto loop in `${CLAUDE_PLUGIN_ROOT}/skills/shared/hitl-loop.md` and its always-ask actions (M9)). Note: a per-run "report only" opt-out (M10) is conversational, never on disk - the orchestrator's dispatch prompt states whether it is in effect for this run
