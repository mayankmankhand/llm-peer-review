...
Render the review page whenever **either** is true:
...
1. **Build the JSON payload** matching the schema documented at the top of `${CLAUDE_PLUGIN_ROOT}/skills/shared/shells/review-shell.html`. Read that header comment for the authoritative field list (it is the single source of truth). Compact reference:
   ...
   - `lenses`: `[string]` - the lenses this run checked, by specialist name: `["code"]` for a direct `/tk:review-code` run, the dispatched set for `/tk:review`. The renderer carries every open finding on the previous page whose lenses are all outside this list forward, marked `carried`, ranked and budgeted with the rest, and counts it as still open; only a finding from a lens that ran can be reported resolved. Omit the key only for a run that checked everything (`/tk:review-full`): with no `lenses`, the whole page is replaced. **Each finding's `specialist` value (or, failing that, its group label) must be the same token as its lens in `lenses`**: `code`, `security`, `security-audit`, `ux`, `browser`, `commands`, `deps`, `copy`, `plan`. The renderer matches lenses against the tokens in that value, so a finding labeled with words that never include its lens token is carried on every later run instead of resolving (review of the #162 cycle, R14).
   ...
   - `groups`: `[{label, findings:[...]}]` grouped by specialist. Each finding: `{id, severity, specialist, file:{relPath, absPath, line}, what, context, fix, locus, receipt, fields:[{label, value}]}`. `severity` is `"block" | "warn" | "suggest"`.
     ...
     - `receipt`: `{cmd, stdoutFile, exit}`. `stdoutFile` is the file the tier-1 runner saved the check's output to as it ran, under `reports/receipts/<run-stamp>/` (M2 tier 1 in `hitl-loop.md` names the folder; "Where the report is written" in the shared output template shows the form); **`render-html.js` reads that file itself** and replaces it with a capped `stdout` array, taking the file's own `exit N` last line as the exit code. It refuses a file from anywhere else, a file over 64 KB, or anything that is not a plain file, and drops the receipt with a note on stderr naming the file. Supplying `stdout` inline instead is refused and dropped, because model-typed text presented as machine output is worse than no receipt at all.
...
2. **Write the JSON to a per-run temp file.** Write it as `data.json` in a fresh folder from `mktemp -d /tmp/review-render.XXXXXX`, made and used per "Temporary folders" in `${CLAUDE_PLUGIN_ROOT}/skills/shared/html-outputs.md`; that folder is `<render-dir>` below.
...
When the orchestrator (`/tk:review`) dispatches specialist subagents, those subagents MUST NOT emit their own HTML companion. Only the orchestrator produces HTML for an orchestrator run - this guarantees one combined HTML file per cycle rather than several overlapping ones. Specialist skills only generate HTML when called directly (e.g., `/tk:review-code` on its own).
