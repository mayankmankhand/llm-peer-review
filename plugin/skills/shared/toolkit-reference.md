...
**Default permission mode has limits.** A plugin command's `allowed-tools` grant lasts only until you send your next message, and a stage the loop starts for you ("go" into `/tk:execute`, then `/tk:review` and `/tk:document`, or `/tk:setup` into `/tk:upgrade`) gets no grant of its own. The plugin script rows in the table above cover both, so a project whose `settings.local.json` lacks them (set up before 7.3.1, or a row you deleted) still stops for approval there: re-run `/tk:setup`, or let `/tk:upgrade` report the missing rows. Saving a review receipt's output to a file uses a redirect, which default mode always asks about, and so do a few of the checks `/tk:upgrade`'s receipts run (`node -e`, `awk`, `find`). Approve those prompts when they come.
