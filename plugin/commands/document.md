---
description: "Update Documentation Task"
allowed-tools:
...
- **DESIGN-PROFILE.md** - When this cycle ran a design step, append every direction from the plan's Direction and Directions tried rows (name, seed, the picked one's best critic score, kept or dropped at pick) under Directions tried, and each brief that failed under Prompts to retry on newer models, with the receipt per M8 (the plan's Outcomes and the design digest). What "failed" means is defined once, in the fragment's digest section. Taste notes are already written by `/tk:explore` as they happen; do not duplicate them here.
- **CODEBASE_MAP.md** - Regenerate by invoking `/tk:index` (the command orchestrates the scanner and parallel subagents to produce a fresh semantic map). For projects over 500k tokens or with per-chunk overflow, `/tk:index` will prompt for cost confirmation before spending API tokens - tell the user this may happen and that they can decline to skip the refresh (the prior map remains intact). If `/tk:index` fails or the user declines, leave the existing map untouched and continue with the rest of `/tk:document`. Do not write the map file manually.
...
- `/tk:ask-gpt`, `/tk:ask-gemini`, `/tk:peer-review`, or any other external model
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/scripts/correction-ledger.js --candidates
```
...
would permanently certify a scan that never ran, and `/tk:error-analysis` would later tell
...
Dispatch the `correction-extractor` agent (`subagent_type=tk:correction-extractor`; an unknown
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/scripts/correction-ledger.js --heartbeat --candidate-count <N> --added-count <M>
```
...
Do not run `/tk:error-analysis` from here. Grouping and ranking is a deliberate, user-triggered
...
Generate a one-page HTML summary of what shipped this cycle. Runs on every `/tk:document`, per `${CLAUDE_PLUGIN_ROOT}/skills/shared/html-outputs.md` (default-on).

### Determine the cycle window

The "cycle" is everything since the last `/tk:document` run, tracked by a marker file.
...
Do NOT hand-write the HTML. Produce a JSON payload matching the schema documented at the top of `${CLAUDE_PLUGIN_ROOT}/skills/shared/shells/document-shell.html` (read its header comment for the exact fields); the helper injects it into the prebuilt shell. Contents:
...
```
node ${CLAUDE_PLUGIN_ROOT}/scripts/render-html.js --index-url --name cycle
```

Update the page whose URL comes back; publish a new one when nothing does. Then show it to the user per the **"Viewing the Artifact"** rules in `${CLAUDE_PLUGIN_ROOT}/skills/shared/html-outputs.md`: publish is the primary viewport, the local open is the fallback, and that section holds the whole decision. Pass `--no-abs` to the render above when this session can publish.
