...
**If it does not exist (first run after upgrade or fresh setup):** Tell the user "No codebase map found. Generating one now via `/tk:index` - this is a one-time setup that may take a minute and spawns parallel subagents." Then invoke `/tk:index`. After it completes, read the new map and proceed.
...
1. Dispatch `subagent_type=tk:plan-critic` with the Agent tool. The prompt carries exactly two things: the plan file's path and the exploration's closing summary (direction, decisions, open questions), pasted verbatim. Never the round number, never earlier critiques, never the score you are aiming for.
