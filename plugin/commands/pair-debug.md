...
**If it does not exist (first run after upgrade or fresh setup):** Tell the user "No codebase map found. Generating one now via `/tk:index` - this is a one-time setup that may take a minute." Then invoke `/tk:index`. After it completes, read the new map and proceed.

**If it is malformed or `/tk:index` fails:** Proceed without the map. Logs and repro info are what really drive debugging - the map is helpful context, not a hard requirement.
