---
description: "Create a Parallel Worktree"
---
...
- This command takes no arguments. Worktree names are auto-generated. The branch gets a meaningful name later when `/tk:explore` identifies an issue.
...
- If `CODEBASE_MAP.md` does not exist in the main repo, skip and note it -
  /tk:explore in the worktree will detect the missing map and generate one
  automatically via /tk:index (one-time, may take a minute).
- If it already exists in the worktree, skip (don't overwrite).
