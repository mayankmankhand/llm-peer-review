---
description: "Ask Gemini - Automated AI Peer Review (Gemini)"
allowed-tools:
  - "Bash(bash ${CLAUDE_PLUGIN_ROOT}/scripts/open-artifact.sh *)"
  - "Bash(bash ${CLAUDE_PLUGIN_ROOT}/scripts/open-artifact.sh)"
  - "Bash(mktemp -d /tmp/*)"
  - "Bash(node ${CLAUDE_PLUGIN_ROOT}/scripts/ask-gemini.js *)"
  - "Bash(node ${CLAUDE_PLUGIN_ROOT}/scripts/ask-gemini.js)"
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/scripts/ask-gemini.js session
```
...
3. If multiple files exist (another parallel `/tk:ask-gemini` tab is running), do NOT just pick the most recent - that file may belong to the other tab and was touched more recently. Ask the user which session ID was echoed back in Step 0, or read the first line of each candidate to find the match.
...
**Why this matters:** two parallel Cursor or Claude Code tabs running `/tk:ask-gemini` would otherwise clobber each other's `/tmp/ask-gemini-context.md` and `/tmp/ask-gemini-debate.md`. The session ID gives each run its own isolated pair of files.
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/scripts/ask-gemini.js review --context-file /tmp/ask-gemini-context-<session-id>.md --review-type [plan|code|branch|feature]
```
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/scripts/ask-gemini.js respond --context-file /tmp/ask-gemini-context-<session-id>.md --debate-file /tmp/ask-gemini-debate-<session-id>.md
```
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/scripts/ask-gemini.js summary --context-file /tmp/ask-gemini-context-<session-id>.md --debate-file /tmp/ask-gemini-debate-<session-id>.md
```

## Step 6: Present Results to User

Present the summary to the user in this format. Each Recommended Action uses the same two-sentence contract as `/tk:review` findings (sentence one, an optional sentence two, then a fix line), defined in `${CLAUDE_PLUGIN_ROOT}/skills/shared/finding-contract.md`, with 🚫/⚠️/💡 emojis and sequential R-IDs - the reasoning is mined from the debate transcript. Agreed Points, Disagreed Points, and Key Insights stay as terse bullets.
