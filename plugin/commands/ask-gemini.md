...
3. If multiple files exist (another parallel `/tk:ask-gemini` tab is running), do NOT just pick the most recent - that file may belong to the other tab and was touched more recently. Ask the user which session ID was echoed back in Step 1.5, or read the first line of each candidate to find the match.
