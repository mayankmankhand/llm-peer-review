...
**Use this when:** You have feedback from `/tk:ask-gpt`, `/tk:ask-gemini`, or another AI model and want to evaluate which findings are real.
**Don't use this when:** You want a first review (use `/tk:review-code`), or you want to start a debate (use `/tk:ask-gpt` or `/tk:ask-gemini`).
...
Write each confirmed finding in the two-sentence contract from `${CLAUDE_PLUGIN_ROOT}/skills/shared/finding-contract.md`: one sentence carrying the defect and its harm, an optional second, then a **Fix:** line naming the cost.
...
The evaluation above is itself the audit step of the auto loop (M2's skeptical lens applied to external feedback); keep its evidence discipline exactly as described. The operating rules live in `${CLAUDE_PLUGIN_ROOT}/skills/shared/hitl-loop.md` (rule IDs M1-M15). Once the verdicts are in:
