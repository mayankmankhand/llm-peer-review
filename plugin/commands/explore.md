...
1. **Read `DESIGN-PROFILE.md`.** Absent means the design system is unknown; offer to create the file from `${CLAUDE_PLUGIN_ROOT}/skills/shared/design-profile-template.md` (the installer's own seed). While the profile says unknown, run the detection signals, confirm once, and record the answer in the profile (the three-state rule).
