...
- **Setup seeds a row for every plugin script** (#185, #188). A plugin command's script grant ended at the next user message, and a stage the loop started on its own got none, so in default permission mode a toolkit script stopped for approval there. The seed now carries every script rule the plugin grants in two spellings: at the plugin cache folder, filled in by `/tk:setup` with this machine's absolute folder, and at the stable `~/.claude/plugins/data/tk-llm-peer-review/current` path, which also covers the push check run outside any command. Measured in a default-mode headless cycle (setup, explore, plan, execute, review, document): 0 toolkit-script prompts. The first form of these rows began with a wildcard, which let `node -e "<code>" <fake path>` run unasked; the chained review found it, the rows now start with a literal folder, and the injection was measured denied. A plugin root outside the plugin cache gets no cache rows.
- **Force pushes always ask** (#192). Six ask rows cover `--force` forms, `-f`, `-uf` and a `+` refspec. Ask takes precedence over the `git push` allow row: measured, 8 force forms asked and 6 normal pushes ran. `git push --mirror` is not covered yet (#196). Both installers merge the ask rows into copy-installs too.
- **Retired rows are removed in one step, and `npm install *` is left alone** (#186, #198). `Bash(npm install *)` is off the retired list, so a project that adds packages with `npm install <name>` keeps it on every upgrade; every other retired row is one C-9 finding. C-9 also reports missing ask rows.
- **C-11 skips history** (#187). Lines in `LESSONS.md` and `LESSONS-detail.md` that start with a date or a version are not flagged for naming an old command.
- **Debate scripts on `openai` 7 and `@google/genai` 2** (#193). `openai` 7 needs Node.js 22 or newer; README and SETUP say so. A live debate round against each provider was not run for this release: the configured keys were rejected. A Node version check is tracked in #197.

### Fixed

- **The push check catches a secret whose value holds the other quote** (#191). The value is also read to the quote that opened it; the old rule stays beside the new one, so no earlier hit is lost.
- **The push check's hook form warns when it gets no ref lines** (#190) instead of passing silently; it still exits 0.
- **`browse.js` uses the Windows temp folder** on Windows for screenshots and its PID file (#194); `/tmp` stays elsewhere. README says browser QA is not yet supported on native Windows.

### Added

- **CONTRIBUTING: isolating headless verification sessions** (#189), with an example `headless-session.sh` run and the split proof for flows that edit `.claude/` files.

### Upgrading

**Claude Code users:** `claude plugin marketplace update llm-peer-review`, then `claude plugin update tk@llm-peer-review`, and restart Claude Code. Then, in each project, run `/tk:setup` again to add the new script and ask rows (it keeps every row of yours and never re-adds one you deleted), and `/tk:upgrade`.

**No new conventions.** C-9 gains the missing-ask-rows finding; C-11 gains the history exemption.

**Other editors:** re-run `setup.sh` or `setup.ps1` to merge the ask rows. Node.js 22 or newer for the debate scripts.

**Suites at the tag:** browse-spawn 21, build-plugin 170, correction-ledger 86, debate-session 18, env-local 52, gen-media 133, generate-index 66, pre-push-check 179, release-check 156, render-html 308, session-init 87, session-start 98, setup-project 303, upgrade-audit 365 checks; installer guarantees 153; `build-plugin.js --check` clean at 122 files.

---

## v7.3.0 - Everything Since 6.3.3, Checked (2026-09-16)
...
**Audit counts (2026-09-16).** 92 commits and 141 source files between the tags, read in 20 shards plus one cross-area pass: 380 shard rows and 9 cross-area rows, every receipt rerun; 32 candidate regressions judged by a fresh skeptic per area; 22 distinct regressions survived (with the narrower claims carved out of refuted rows) and 11 rows were audited out. All 22 are fixed in this release; most trace to one root, the 7.0.0 plugin build rewriting names and permissions for the plugin path and leaving the copy-install path behind. Suites at HEAD eacea72, every one exit 0: browse-spawn 18, build-plugin 168, correction-ledger 86, debate-session 18, env-local 52, gen-media 133, generate-index 66, pre-push-check 173, release-check 156, render-html 308, session-init 86, session-start 98, setup-project 293, upgrade-audit 349; installer guarantees 152; `build-plugin.js --check` clean.

### Changed

- **A review typed with no range starts from the plan** (#184). `/review` with no range used to take your newest 20 unpushed commits and never look at the plan, so on a pushed branch it found nothing and after a long cycle it dropped the oldest commits. `session-init.js --scope` now consults the newest plan's `**Start commit:**` first and uses it while at least one commit after it is unpushed, with no cap however long the span (a range argument narrows it); a plan whose commits are all pushed counts as shipped, and the review stops and names the range to pass. The first line always says which source it used and why. The 20-commit fallback stays for a project with no plan that applies. The shipped-plan message names the range to pass without a command form, and the plan is read under the git root, so a review typed from a subfolder finds it (review of this cycle, R2 and R7).
...
- **`scripts/setup/headless-session.sh`** (maintainer-only): runs a headless session against a plugin build under a scratch home, so a product-level test never touches the real `~/.claude`; `--baseline` and `--verify` prove it. It passes the build folder as an allowed directory too: a plugin command inlines fragments from the plugin root, and a `--plugin-dir` outside the plugin cache is blocked from `cat` until the session is told about it (the first #184 scenario run ended at turn zero on exactly that). After the run it checks that the credentials link survived, since a token refresh that replaced it would leave the real login on a stale token (review of this cycle, R8).

### Upgrading

**Claude Code users:** `claude plugin marketplace update llm-peer-review`, then `claude plugin update tk@llm-peer-review`, and restart Claude Code. A project with a project-scope install entry updates it from that project with `claude plugin update tk@llm-peer-review --scope project`. Then run `/tk:upgrade` in each project: C-7 carries the install-neutral rules text, and the prompts that changed are in the plugin itself.

**No new conventions.**

**Other editors:** re-run `setup.sh` or `setup.ps1`. It adds the four permission rows the 7.x prompts call, backs up and removes the old `.claude/rules/html-outputs.md`, and replaces the changed scripts.

**Suites at the tag:** browse-spawn 18, build-plugin 168, correction-ledger 86, debate-session 18, env-local 52, gen-media 133, generate-index 66, pre-push-check 173, release-check 156, render-html 308, session-init 87, session-start 98, setup-project 293, upgrade-audit 349 checks; installer guarantees 152; `build-plugin.js --check` clean at 122 files.
