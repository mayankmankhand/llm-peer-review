...
- **The hosted page is now the primary viewport for HTML artifacts (#155).** Plans, reviews, cycle summaries and the rest are published to a private Claude-hosted page and you get the link; the local browser open is the fallback, used when the session cannot publish (Cursor, or the feature is off) or when you decline the consent ask. Exactly one viewport opens either way, so you are never handed a bare file path. Review and debate artifacts still ask for consent every single run.
