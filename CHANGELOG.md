...
- **The GitLab half of this has never been run** (#143) - Every `glab` command shipped here was written from glab's published CLI documentation and executed zero times, because there is no `glab` on the machine the toolkit was authored on. The reference file says so itself, and names the two most likely places it breaks: `--output json` has moved between glab versions (older builds want `-F json`), and `--jq` is a `gh` flag that `glab` does not have, which is why the GitLab merge lookup pipes into `jq` instead. If both fail, the commands fall back to plain `git log`. One more thing GitLab users should expect: the `glab` permission entries are documented in the toolkit rules but deliberately not seeded into a fresh install, so you either add the ones you use to your own `.claude/settings.local.json` or approve each call at the prompt. GitHub behavior is unchanged and remains the tested path.
