...
- **Two guards, because the bug was invisible to every layer that existed.** The defect read as a comment to anyone scanning the source, rendered without complaint (the helper only checked its two placeholders), and no test read the shells. Now `render-html.js` refuses to render a shell whose header comment contains another comment opener, naming the shell and the line, which is the layer that protects a downstream project that hand-edits a shell. And `scripts/test-render-html.js` asserts, for all seven shells, no nested comment, each placeholder exactly once, and the render-data island present, then renders a plan and reads the page the way a browser does to assert nothing visible sits above the title, and finally runs a copied helper against a deliberately broken shell to prove the refusal fires. The suite grows from 188 to 223 checks; every new check was mutation-tested by restoring the bug and confirming three checks fail naming `plan-shell.html:18`.
