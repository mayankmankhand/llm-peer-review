...
- **Five patterns.** `gitlab-pat` for `glpat-`, `gitlab-token` for GitLab's deploy, runner, service-account, pipeline-trigger and cluster-agent tokens, `npm-token` for npm access tokens, `jwt` for JSON Web Tokens, and `netrc-record` for a netrc credential record pasted somewhere that is not a `.netrc`. Nine patterns become fourteen.
