...
- **HTML option comparison (when 2+ options being compared)** - skip this entirely when fewer than 2 options are on the table. When it applies, work through these steps:
  - **Announce** per `.claude/rules/html-outputs.md`: "Generating an HTML view because you are comparing N options. Say 'skip HTML' if you want markdown only."
  - **Generate the canonical record** - a persisted static side-by-side HTML at `artifacts/html/explore-{topic-slug}-{YYYY-MM-DD}.html` capturing the option cards, scope-dial outcome, and key decisions. This file is the canonical record of the exploration. For the visual look, **read** `.claude/skills/shared/html-look.md` only when generating (not on every run) and inline its tokens (typography, color tokens, severity hex) into the `<style>` block.
