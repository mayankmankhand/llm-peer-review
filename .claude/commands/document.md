...
Write the accepted rows as `rows.json` in a fresh folder from `mktemp -d /tmp/correction-rows.XXXXXX`, made and used per "Temporary folders" in `.claude/skills/shared/html-outputs.md` (inlined at the end of this file), then append them. Two parallel sessions must never share the file: the script deletes it after a successful `--add`, so a shared name lets one session eat the other's rows (holistic review, R21).

```bash
node .claude/scripts/correction-ledger.js --add --data <folder>/rows.json
```
...
- **Push** behind the M11 tripwire: run `node .claude/scripts/pre-push-check.js <remote> <branch>` for the branch being pushed (usually `origin` and the current branch name), follow M11's exit-code consequences (`.claude/skills/shared/hitl-loop.md`), and on a clean check push that same destination with `git push -u <remote> HEAD:<branch>`.
