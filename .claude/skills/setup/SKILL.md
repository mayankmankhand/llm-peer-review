...
3. **Read the exit code.**
   - **0, done.** Relay the report verbatim. When the run changed anything, it ends with a one-line undo (after a migration, `git checkout` plus the backup folder; after a fresh or plugin run, the files to delete and any tracked files to check out) and, after a migration, with "Next: run /upgrade".
