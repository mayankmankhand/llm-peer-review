...
3. **Read the exit code.**
   - **0, done.** Relay the report verbatim. When the run changed anything, it ends with a one-line undo that restores the project when followed left to right (tracked files to check out, then files to delete, then files to copy back from the backup folder or restore by hand) and, after a migration, with "Next: run /upgrade".
