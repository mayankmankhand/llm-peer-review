...
`/setup` merges a fixed baseline into the project's `.claude/settings.local.json`: every allow row in the table below, exactly as written there, plus `additionalDirectories: ["/tmp"]`. It adds a row only when no permissions list in the file holds it (allow, ask or deny, with `Bash(x:*)` and `Bash(x *)` counting as one row) and it has not offered that row in this working copy before, sets no `defaultMode`, and writes no row for a script in the project's own `.claude/scripts/`: the `node .claude/scripts/...` rows a copy-install needed are dead under the plugin, and setup removes the ones whose script the project no longer has, plus a few old toolkit rows. It does write rows for the plugin's own scripts (in the table below), because a command's `allowed-tools` grant ends when you send your next message and a stage the loop starts for you gets none (issue #185). It also merges the baseline's ask rows into the file's ask list the same way (issue #192).
...
| `Bash(npm install)`, `Bash(npm uninstall *)` | A plain `npm install` of the project's own dependencies, and removing a package. Any other install, the one `/worktree` runs inside a new worktree included, asks once, because an install can run a package's own install scripts and a wildcard row would also allow added package names |
...
| `Bash(bash */.claude/plugins/cache/llm-peer-review/tk/*/scripts/open-artifact.sh *)`, `Bash(bash */.claude/plugins/cache/llm-peer-review/tk/*/scripts/open-artifact.sh)`, `Bash(cat * \| node */.claude/plugins/cache/llm-peer-review/tk/*/scripts/browse.js *)`, `Bash(echo * \| node */.claude/plugins/cache/llm-peer-review/tk/*/scripts/browse.js *)`, `Bash(node */.claude/plugins/cache/llm-peer-review/tk/*/scripts/ask-gemini.js *)`, `Bash(node */.claude/plugins/cache/llm-peer-review/tk/*/scripts/ask-gemini.js)`, `Bash(node */.claude/plugins/cache/llm-peer-review/tk/*/scripts/ask-gpt.js *)`, `Bash(node */.claude/plugins/cache/llm-peer-review/tk/*/scripts/ask-gpt.js)`, `Bash(node */.claude/plugins/cache/llm-peer-review/tk/*/scripts/browse.js *)`, `Bash(node */.claude/plugins/cache/llm-peer-review/tk/*/scripts/browse.js)`, `Bash(node */.claude/plugins/cache/llm-peer-review/tk/*/scripts/correction-ledger.js *)`, `Bash(node */.claude/plugins/cache/llm-peer-review/tk/*/scripts/correction-ledger.js)`, `Bash(node */.claude/plugins/cache/llm-peer-review/tk/*/scripts/gen-media.js *)`, `Bash(node */.claude/plugins/cache/llm-peer-review/tk/*/scripts/gen-media.js)`, `Bash(node */.claude/plugins/cache/llm-peer-review/tk/*/scripts/generate-index.js *)`, `Bash(node */.claude/plugins/cache/llm-peer-review/tk/*/scripts/generate-index.js)`, `Bash(node */.claude/plugins/cache/llm-peer-review/tk/*/scripts/pre-push-check.js *)`, `Bash(node */.claude/plugins/cache/llm-peer-review/tk/*/scripts/pre-push-check.js)`, `Bash(node */.claude/plugins/cache/llm-peer-review/tk/*/scripts/render-html.js *)`, `Bash(node */.claude/plugins/cache/llm-peer-review/tk/*/scripts/render-html.js)`, `Bash(node */.claude/plugins/cache/llm-peer-review/tk/*/scripts/session-init.js *)`, `Bash(node */.claude/plugins/cache/llm-peer-review/tk/*/scripts/session-init.js)`, `Bash(node */.claude/plugins/cache/llm-peer-review/tk/*/scripts/setup-project.js *)`, `Bash(node */.claude/plugins/cache/llm-peer-review/tk/*/scripts/setup-project.js)`, `Bash(node */.claude/plugins/cache/llm-peer-review/tk/*/scripts/upgrade-audit.js *)`, `Bash(node */.claude/plugins/cache/llm-peer-review/tk/*/scripts/upgrade-audit.js)` | The toolkit's own scripts at the path a plugin command types: the plugin's versioned cache folder, for any home folder and any version, so an update needs no new row. Without them a script run after you answer one of a command's questions, or in a stage the loop starts for you, stops for approval (issue #185). Each script has a row with and without arguments; the two `browse.js` pipe rows feed it a JSON action list |
| `Bash(bash ~/.claude/plugins/data/tk-llm-peer-review/current/scripts/open-artifact.sh *)`, `Bash(bash ~/.claude/plugins/data/tk-llm-peer-review/current/scripts/open-artifact.sh)`, `Bash(cat * \| node ~/.claude/plugins/data/tk-llm-peer-review/current/scripts/browse.js *)`, `Bash(echo * \| node ~/.claude/plugins/data/tk-llm-peer-review/current/scripts/browse.js *)`, `Bash(node ~/.claude/plugins/data/tk-llm-peer-review/current/scripts/ask-gemini.js *)`, `Bash(node ~/.claude/plugins/data/tk-llm-peer-review/current/scripts/ask-gemini.js)`, `Bash(node ~/.claude/plugins/data/tk-llm-peer-review/current/scripts/ask-gpt.js *)`, `Bash(node ~/.claude/plugins/data/tk-llm-peer-review/current/scripts/ask-gpt.js)`, `Bash(node ~/.claude/plugins/data/tk-llm-peer-review/current/scripts/browse.js *)`, `Bash(node ~/.claude/plugins/data/tk-llm-peer-review/current/scripts/browse.js)`, `Bash(node ~/.claude/plugins/data/tk-llm-peer-review/current/scripts/correction-ledger.js *)`, `Bash(node ~/.claude/plugins/data/tk-llm-peer-review/current/scripts/correction-ledger.js)`, `Bash(node ~/.claude/plugins/data/tk-llm-peer-review/current/scripts/gen-media.js *)`, `Bash(node ~/.claude/plugins/data/tk-llm-peer-review/current/scripts/gen-media.js)`, `Bash(node ~/.claude/plugins/data/tk-llm-peer-review/current/scripts/generate-index.js *)`, `Bash(node ~/.claude/plugins/data/tk-llm-peer-review/current/scripts/generate-index.js)`, `Bash(node ~/.claude/plugins/data/tk-llm-peer-review/current/scripts/pre-push-check.js *)`, `Bash(node ~/.claude/plugins/data/tk-llm-peer-review/current/scripts/pre-push-check.js)`, `Bash(node ~/.claude/plugins/data/tk-llm-peer-review/current/scripts/render-html.js *)`, `Bash(node ~/.claude/plugins/data/tk-llm-peer-review/current/scripts/render-html.js)`, `Bash(node ~/.claude/plugins/data/tk-llm-peer-review/current/scripts/session-init.js *)`, `Bash(node ~/.claude/plugins/data/tk-llm-peer-review/current/scripts/session-init.js)`, `Bash(node ~/.claude/plugins/data/tk-llm-peer-review/current/scripts/setup-project.js *)`, `Bash(node ~/.claude/plugins/data/tk-llm-peer-review/current/scripts/setup-project.js)`, `Bash(node ~/.claude/plugins/data/tk-llm-peer-review/current/scripts/upgrade-audit.js *)`, `Bash(node ~/.claude/plugins/data/tk-llm-peer-review/current/scripts/upgrade-audit.js)` | The same scripts at the stable path `~/.claude/plugins/data/tk-llm-peer-review/current/`, which a request made outside a command types, for example the push check that runs before "push this" (M11, issue #188) |
...
**Default permission mode has limits.** A plugin command's `allowed-tools` grant lasts only until you send your next message, and a stage the loop starts for you ("go" into `/execute`, then `/review` and `/document`, or `/setup` into `/upgrade`) gets no grant of its own. The plugin script rows in the table above cover both, so a project whose `settings.local.json` lacks them (set up before 7.3.1, or a row you deleted) still stops for approval there: re-run `/setup`, or let `/upgrade` report the missing rows. Saving a review receipt's output to a file uses a redirect, which default mode always asks about, and so do a few of the checks `/upgrade`'s receipts run (`node -e`, `awk`, `find`). Approve those prompts when they come.
