...
| `Bash(bash <tk-plugin-cache>/*/scripts/open-artifact.sh *)`, `Bash(bash <tk-plugin-cache>/*/scripts/open-artifact.sh)`, `Bash(cat * \| node <tk-plugin-cache>/*/scripts/browse.js *)`, `Bash(echo * \| node <tk-plugin-cache>/*/scripts/browse.js *)`, `Bash(node <tk-plugin-cache>/*/scripts/ask-gemini.js *)`, `Bash(node <tk-plugin-cache>/*/scripts/ask-gemini.js)`, `Bash(node <tk-plugin-cache>/*/scripts/ask-gpt.js *)`, `Bash(node <tk-plugin-cache>/*/scripts/ask-gpt.js)`, `Bash(node <tk-plugin-cache>/*/scripts/browse.js *)`, `Bash(node <tk-plugin-cache>/*/scripts/browse.js)`, `Bash(node <tk-plugin-cache>/*/scripts/correction-ledger.js *)`, `Bash(node <tk-plugin-cache>/*/scripts/correction-ledger.js)`, `Bash(node <tk-plugin-cache>/*/scripts/gen-media.js *)`, `Bash(node <tk-plugin-cache>/*/scripts/gen-media.js)`, `Bash(node <tk-plugin-cache>/*/scripts/generate-index.js *)`, `Bash(node <tk-plugin-cache>/*/scripts/generate-index.js)`, `Bash(node <tk-plugin-cache>/*/scripts/pre-push-check.js *)`, `Bash(node <tk-plugin-cache>/*/scripts/pre-push-check.js)`, `Bash(node <tk-plugin-cache>/*/scripts/render-html.js *)`, `Bash(node <tk-plugin-cache>/*/scripts/render-html.js)`, `Bash(node <tk-plugin-cache>/*/scripts/session-init.js *)`, `Bash(node <tk-plugin-cache>/*/scripts/session-init.js)`, `Bash(node <tk-plugin-cache>/*/scripts/setup-project.js *)`, `Bash(node <tk-plugin-cache>/*/scripts/setup-project.js)`, `Bash(node <tk-plugin-cache>/*/scripts/upgrade-audit.js *)`, `Bash(node <tk-plugin-cache>/*/scripts/upgrade-audit.js)` | The toolkit's own scripts at the path a plugin command types: the plugin's versioned cache folder. Setup writes `<tk-plugin-cache>` as this machine's own folder (for example `/home/you/.claude/plugins/cache/llm-peer-review/tk`), and the `*` after it stands for any version, so an update needs no new row. The full folder in front keeps a row from matching a command that only ends in that path, such as inline code placed before it. Setup leaves these rows out when the plugin does not run from the cache (a `--plugin-dir` build). Without them a script run after you answer one of a command's questions, or in a stage the loop starts for you, stops for approval (issue #185). Each script has a row with and without arguments; the two `browse.js` pipe rows feed it a JSON action list |
