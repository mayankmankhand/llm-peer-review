...
Write your action sequence as `actions.json` in a fresh folder from `mktemp -d /tmp/browse-run.XXXXXX`, made and used per "Temporary folders" in `.claude/skills/shared/html-outputs.md` (two sessions driving a browser at once never read each other's actions), then pipe that file to the script (`<run-dir>` is the folder `mktemp` printed):

```bash
cat <run-dir>/actions.json | node .claude/scripts/browse.js
```
