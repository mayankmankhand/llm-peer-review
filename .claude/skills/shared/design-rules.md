...
1. Run `node .claude/scripts/gen-media.js --kind seed` and take the `seed` field. On the plugin, the commands that run a design step carry their own permission for the script, so the call should not prompt; if it does, the session has not loaded the current plugin, so run `/reload-plugins`. On a copy-install, setup seeds the row into `.claude/settings.local.json`, and an install that predates that seeding gets it by re-running setup. Either way the user can also add the row by hand, since Claude cannot edit that file. Do not reach for a shell one-liner.
