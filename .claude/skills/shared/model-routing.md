...
All four agents declare a `tools` list granting no Edit, Write, or NotebookEdit: a finder that could edit would apply changes before the M2 audit judged them, bypassing the loop. `review-finder` also carries Bash, which the Browser QA and Dependency Security specialists need to run their own tooling; `index-mapper`, which only reads its chunk, does not, and neither does `correction-extractor`, which reads only the pre-filtered candidate list handed to it, nor `design-critic`, which reads one screenshot path and nothing else (no Bash, so it cannot go looking for the code it is judging).

Everything else inherits: the M2 and M3 audit and verify agents, the `design-critic` judge, `/execute` implementers, `/security-audit` area agents, ad hoc research subagents, and every main-loop stage. When dispatching an inherit role, omit the model parameter entirely rather than pinning a top-tier ID - a hardcoded top-tier pin goes stale across model generations and can be blocked by an org policy.
