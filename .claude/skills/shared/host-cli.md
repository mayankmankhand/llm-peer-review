...
|---|---|---|
| Create issue | `gh issue create --title '<title>' --body-file <body-file> --label 'L'` | `glab issue create --title '<title>' --description-file <body-file> --label 'L'` |
| Read issue | `gh issue view <N> --json title,body` -> fields `title`, `body` | `glab issue view <N> --output json` -> fields `title`, `description` |
| Create PR / MR | `gh pr create --base main --title '<title>' --body-file <body-file>` | `glab mr create --target-branch main --title '<title>' --description-file <body-file>` |
...
- **The GitLab column has been executed** against `glab 1.115.0` on a live repo (~270 issues, 31 merged MRs): the read, list and merged rows as written, and both create rows in their inline `--description '<body>'` form, which is now the fallback. The `--description-file` form follows glab's current documentation for `issue create` and `mr create` ("Read the issue description from a file") and has not been executed here. GitHub-style `--body` / `--base` correctly error with `Unknown flag`. One version caveat remains untested: `--output json` has moved between glab builds, and older ones want `-F json`.
