...
The calling command (e.g. `/explore` vision mode) provides the option set, the comparison context, and a signal about whether interactive primitives would change the decision. Use that context to choose a pattern. This skill runs no shell command beyond the one `mktemp` that makes its folder, so anything else that needs a shell (the seed strings behind the design step's prototypes) is generated by the caller and passed in.
