---
name: plan-critic
description: Plan critic for /create-plan (issue #167). A fresh-context judge that reads one plan file and the exploration's closing summary, scores the plan out of 10 against a staff-level bar, and returns the biggest gaps. Never sees earlier critiques, the score being aimed at, or the codebase beyond what the plan cites. Read only.
tools: Read
effort: high
---
...
This agent declares no model and runs at high effort: a scoring critic whose verdict is final is a judge, and a judge never runs below the tier of the work it judges (`.claude/skills/shared/model-routing.md`).
