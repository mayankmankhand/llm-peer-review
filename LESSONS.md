...
- **Version bumps touch more files than you think.** When releasing v3.4, the reviews caught 5 files that needed version updates. By v4.0 the count grew to 6: VERSION, package.json, package-lock.json, CHANGELOG.md, AGENT-SETUP.md title + "What's new" section, and .claude/rules/toolkit.md version comment. Easy to miss one. No automated fix for the toolkit repo (the checklist would leak to downstream users), so just remember to check all six manually. Issue #78 proved this lesson again: PR #77 shipped `/review-copy` with none of the bookkeeping done, and even the fix plan initially missed package.json and toolkit.md.
