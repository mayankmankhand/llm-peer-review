...
Setup installs from `main`, not from a tag, so this list tracks what you actually get today. Items landed since the last tag are marked *(unreleased)*; see the CHANGELOG for the full split.
...
- **It works on GitLab now, not just GitHub.** Commands read your git remote and pick `gh` or `glab` from what they find. Worth knowing *(unreleased)*: the GitLab path has now been run end to end against a live repo, and the three defects that turned up are fixed. One caveat is still untested: `--output json` has moved between glab versions, and older builds want `-F json`. GitHub is unchanged.
- **HTML artifacts open as a private hosted page.** *(unreleased)* Plans, reviews, and cycle summaries are published to a private Claude-hosted page and you get the link. When your session cannot publish, or you decline the consent ask, the same file opens in your local browser instead - so you always get exactly one viewport, never a bare file path. Published pages no longer carry the absolute paths of files on your machine; file references are repo-relative.
