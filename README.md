...
**Latest release: v6.2.0** (September 2026), additive on top of **v6.1.1**: it adds the design workflow described under `/explore` above, with a design critic loop in `/execute`, optional media generation behind your own keys, and a user-owned `DESIGN-PROFILE.md` seeded once by setup (#160); [What v6.2.0 adds](#what-v620-adds) has the bullets. v6.1.1 was a one-fix patch on **v6.1.0** (the plan shell's header comment rendering as page text, plus the three-vote downgrade, #158 and #159). v6.1.0 itself is additive on top of **v6.0.0** (August 2026). **v6.0.0 is the one that changed behavior** and is still the release to read first: if you are upgrading from v5.x or earlier, start with its bullets directly below.

Setup installs from `main`, not from a tag, so you get everything on this page, not just the tagged release. The bullets directly below describe v6.0.0; [What v6.1.0 adds](#what-v610-adds) and [What v6.2.0 adds](#what-v620-adds) list what the newer releases build on top of it. See the CHANGELOG for the full split.
...
| `.claude/scripts/` (`ask-gpt.js`, `ask-gemini.js`, `browse.js`, `correction-ledger.js`, `gen-media.js`, `generate-index.js`, `open-artifact.sh`, `pre-push-check.js`, `render-html.js`, `session-init.js`, `package.json`, `package-lock.json`) | `your-project/.claude/scripts/` |
