...
skills:
  ...
  - skills/worktree
---
<!-- GENERATED by node .claude/scripts/build-layouts.js from .claude/agents/correction-extractor.md. Do not edit: change the source under .claude/ and rebuild. -->

Tool names here: this agent was written for Claude Code, so where the text below says Read, Grep, or Bash, use view_file, grep_search, or run_command. No write or replace tool is ever granted to this agent.
