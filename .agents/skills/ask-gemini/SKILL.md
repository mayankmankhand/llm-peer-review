---
name: ask-gemini
description: "Getting a structured second opinion from Gemini via a debate of up to 3 rounds, on a plan, code change, branch, or feature. Not when: You want a first-pass code review (use `/review-code`), or you want to evaluate findings from a prior debate (use `/peer-review`). Typed by a human only, never invoked implicitly (M14)."
disable-model-invocation: true
---
<!-- GENERATED by node .claude/scripts/build-layouts.js from .claude/commands/ask-gemini.md. Do not edit: change the source under .claude/ and rebuild. -->
