...
---
<!-- GENERATED by node .claude/scripts/build-layouts.js from .claude/skills/review-commands/SKILL.md. Do not edit: change the source under .claude/ and rebuild. -->
...
Read the command files being reviewed. Skip any file whose first lines carry `GENERATED by node .claude/scripts/build-layouts.js` (the derived copies under `.agents/`, `.codex/`, and `.cursor/`, issue #144) and review the source it names instead: a fix to a copy is overwritten by the next build. Then pick one of two modes:
