---
name: create-plan
description: "Turning a fully-explored idea into a step-by-step implementation plan with status tracking. Not when: The idea is not yet scoped (use `/explore` first), or the change is small enough that a plan would just be ceremony."
---
<!-- GENERATED by node .claude/scripts/build-layouts.js from .claude/commands/create-plan.md. Do not edit: change the source under .claude/ and rebuild. -->
